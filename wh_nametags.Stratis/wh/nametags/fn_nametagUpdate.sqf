...
if(WH_NT_NAMETAGS_ON) then
{
	...
	private ["_ppos","_pos","_ents","_dist","_range","_veh","_i","_role","_fov",
			"_vehS","_angle","_noRG"];

	// Find player camera's position. Allows this script to work with 3PP.
	_ppos = if (cameraView isEqualTo "EXTERNAL") then 
	{ positionCameraToWorld[0,0,0] } else { getPosATL player };
	...
	_fov = if ( WH_NT_DRAWDISTANCE_FOV ) then 
