	...
	_entity = _x;
	{
		...
		if !(_x isEqualTo _player) then
		{
			...
			else
			{
				...
				_vehicle = _entity; //objectParent _x
			...
			if _show then
			{
				...
				_unitData = [];
				_unitData pushBack _entity;
				_unitData pushBack _name;				// Index 0
				_unitData pushBack _nameColor;			// Index 1
				_unitData pushBack _locationData;		// Index 2
				_unitData pushBack _role;				// Index 3
				_unitData pushBack _groupName;			// Index 4
				_unitData pushBack _drawRoleAndGroup;	// Index 5
